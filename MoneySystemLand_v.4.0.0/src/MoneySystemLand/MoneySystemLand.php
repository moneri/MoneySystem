...
class MoneySystemLand extends PluginBase implements Listener{
    ...
    **/
	...
	public function onEnable(){
        ...
        $this->road = new Config($this->getDataFolder() . "RoadBuilder.data", Config::YAML);
        $this->data = new Config($this->getDataFolder() . "Road.yml", Config::YAML);
    ...
    public function onCommand(CommandSender $sender, Command $command, $label, array $args){
        switch($command->getName()){
            case "buildroadstart":
                if(!$sender instanceof Player){
                    $sender->sendMessage(TextFormat::YELLOW."このコマンドはコンソールからは実行できません。");
                    return true;
                }
                if($this->road->exists($sender->getName())){
                    return true;
                }else{
                    $sender->sendMessage(TextFormat::YELLOW."あなたは道路建設者ではありません。");
                    return true;
                }
                if($sender->getLevel()->getFolderName() == "resources" || $sender->getLevel()->getFolderName() == "world"){
                    $sender->sendMessage(TextFormat::YELLOW."生活ワールド以外での道路の建設は、サーバーオーナー(鯖主)でないとできません。");
                    return true;
                }
                $x = (int) $sender->x;
                $z = (int) $sender->z;
                $level = $sender->getLevel()->getFolderName();
                $this->rstart[$sender->getName()] = array("x" => $x, "z" => $z, "level" => $level);
            ...
            case "buildroadend":
                ...
                if($sender->getLevel()->getFolderName() !== $this->rstart[$sender->getName()]["level"]){
                    $sender->sendMessage(TextFormat::YELLOW."道路の設定ワールドが最初と異なります。");
                    return true;
                }
                $x = (int) $sender->x;
                $z = (int) $sender->z;
                $level = $sender->getLevel()->getFolderName();
                $startX = $this->rstart[$sender->getName()]["x"];
                $startZ = $this->rstart[$sender->getName()]["z"];
                $endX = (int) $sender->x;
                $endZ = (int) $sender->z;
                $num = $this->set->get("RoadID");
                $this->data->set($num, [
                "StartX" => (int) $startX,
                "StartZ" => (int) $startZ,
                "EndX" => (int) $endX,
                ...
                $this->data->save();
                $roadid = $num + 1;
                $this->set->set("RoadID", $roadid);
                $this->set->save();
                $sender->sendMessage(TextFormat::GREEN."[MoneySystemLand / MSRoadSystem] 建設した道路を登録しました。");
                return true;
            break;

            case "setroadbuilder":
                if($sender instanceof Player){
                    $sender->sendMessage(TextFormat::YELLOW."このコマンドはコンソールからのみ実行可能です。");
                    return true;
                }else{
                    if(!isset($args[0])){
                        $this->getLogger()->notice("追加するプレイヤー名を入力してください。");
                        return true;
                    }
                    if($this->road->exists($args[0])){
                        $this->getLogger()->info(TextFormat::YELLOW.$args[0]."は、既に登録されています。");
                        return true;
                    }
                    $player = $this->getServer()->getPlayer($args[0]);
                    if($player->isOnline() == NULL){
                        $this->road->set($args[0], "not");
                        $this->road->save();
                        $this->getLogger()->info($args[0]."を道路建設者リストに追加しました。");
                        return true;
                    }
                        $player->sendMessage(TextFormat::GREEN."貴方は道路建設者に任命されました。");
                        $this->road->set($args[0], "okt");
                        $this->road->save();
                        $this->getLogger()->info($args[0]."を道路建設者リストに追加しました。");
                        return true;
                        break;
            ...
            case "removeroadbuilder":
                if($sender instanceof Player){
                    $sender->sendMessage(TextFormat::YELLOW."このコマンドはコンソールからのみ実行可能です。");
                    return true;
                }else{
                    if(!isset($args[0])){
                        $this->getLogger()->notice("削除するプレイヤー名を入力してください。");
                        return true;
                    }
                    if(!$this->road->exists($args[0])){
                        $this->getLogger()->info(TextFormat::YELLOW.$args[0]."は、登録されていません。");
                        return true;
                    }
                    $player = $this->getServer()->getPlayer($args[0]);
                    if($player->isOnline() == NULL){
                        $this->road->remove($args[0], "remove");
                        $this->road->save();
                        $this->getLogger()->info($args[0]."を道路建設者リストから除外しました。");
                        return true;
                    }
                    $player->sendMessage(TextFormat::GREEN."貴方は道路建設者から除外されました。");
                    $this->road->remove($args[0]);
                    $this->road->save();
                    $this->getLogger()->info($args[0]."を道路建設者リストから削除しました。");
                    return true;
                    break;
                }
            ...
            case "startp":
                if(!$sender instanceof Player){
                    ...
                    if($this->set->get("disable-levels") == $sender->getLevel()->getFolderName()){
        ...
        switch($args[0]){
            ...
            case "buy":
                ...
                if(!$sender instanceof Player){
                    ...
                    if(in_array($sender->getLevel()->getFolderName(), $this->set->get("disable.levels"))){
                        ...
                        $this->sys->TakeMoney($sender, $price);
            ...
            case "sell":
                ...
                if($land["Owner"] !== $sender->getName() || !$sender->isOp()){
                    ...
                    $this->land->save();
                    $this->sys->AddMoney($sender, ($price / 2));
            ...
            case "mark":
                ...
                $set = $new;
                $pk->eid = $set;
                $username = TextFormat::GREEN."◆ LandNumber: ".TextFormat::WHITE.$num.TextFormat::GREEN." ◆";
                ...
                $this->marking[$sender->getName()] = true;
                return true;
                break;
            ...
            case "unmark":
                ...
                if(empty($this->marking[$sender->getName()])){
                ...
                $this->marking[$sender->getName()] = null;
                return true;
                break;
            ...
            case "invite":
                ...
                if($this->invite->get($num) == $args[2]){
                    $sender->sendMessage(TextFormat::YELLOW."すでに土地共有をしています。");
                    return true;
                }
                if(!$this->invite->exists($num)){
                    $this->invite->set($num, [
                        $args[2]
                    ]);
                    $this->invite->save();
                ...
                $now = $this->invite->get($num);
                $set = [];
                $data = implode("- ", $now);
                $set[$num][] = $data."\n";
                $set[$num][] = $args[2];
                $this->invite->set($num, $set[$num]);
                $this->invite->save();
                $sender->sendMessage(TextFormat::GREEN."土地番号 #".$num." に ".$args[2]." を招待しました。");
                return true;
                break;
            ...
            case "invitee":
                ...
                $in = $this->invite->get($num);
                if(isset($in[$sender->getName()]) and !$sender->isOp()){
                    $sender->sendMessage(TextFormat::YELLOW."土地番号　#".$num." にあなたは招待されていません。");
                    return true;
                }
                $invitee = $this->invite->get($num);
                if($invitee == null or $invitee == ""){
                    $sender->sendMessage("土地共有しているプレイヤーはいません。");
                    return true;
                }
                $output = "";
                $output .= implode("", $in);
                $sender->sendMessage("=====土地共有リスト=====\n".$output);
                return true;
                break;
            ...
            case "unvite":
                ...
                $now = $this->invite->get($args[1]);
                $after = ltrim($now, " - ".$args[2]);
                $this->invite->set($args[1], $after);
                $this->invite->save();
    ...
    public function onMove(PlayerMoveEvent $event){
        ...
        foreach($data as $road){
            if($level == $road["Level"] and $road["StartX"] <= $x and $road["EndX"] >= $x and $road["StartZ"] <= $z and $road["EndZ"] >= $z){
                $effect = Effect::getEffect(1);
                $effect->setDuration(9999*20);
    ...
    public function onJoin(PlayerJoinEvent $event){
        ...
        if($this->road->exists($name)){
            $road = $this->road->get($name);
            if($road == "not"){
                $player->sendMessage(TextFormat::GREEN."貴方は道路建設者に任命されました。");
                $this->road->set($name, "okt");
                $this->road->save();
                return true;
            }
            if($road == "remove"){
                $player->sendMessage(TextFormat::YELLOW."貴方は道路建設者から除外されました。");
                $this->road->remove($name);
                $this->road->save();
                return true;
            }
        }
    ...
    public function onTouch(PlayerInteractEvent $event){
        ...
        foreach($this->land->getAll() as $land){
            if($level == $land["Level"] and $land["StartX"] <= $x and $land["EndX"] >= $x and $land["StartZ"] <= $z and $land["EndZ"] >= $z){
                $status = $this->invite->get($land["ID"]);
                if($player->getName() === $land["Owner"] or isset($status[$name]) or $player->isOp()){
                    return true;
                }else{
        ...
        $x = $block->getX();
        $z = $block->getZ();
        foreach($this->data->getAll() as $road){
            if($level == $road["Level"] and $road["StartX"] <= $x and $road["EndX"] >= $x and $road["StartZ"] <= $z and $road["EndZ"] >= $z){
                if($player->isOp()){
                    return true;
                }else{
    ...
    public function onBreak(BlockBreakEvent $event){
        ...
        foreach($this->land->getAll() as $land){
            if($level == $land["Level"] and $land["StartX"] <= $x and $land["EndX"] >= $x and $land["StartZ"] <= $z and $land["EndZ"] >= $z){//範囲該当
                $status = $this->invite->get($land["ID"]);
                if($player->getName() === $land["Owner"] or isset($status[$name]) or $player->isOp()){//プレイヤー免除
                    return true;
                }else{
                    $player->sendPopup(TextFormat::RED.TextFormat::BOLD."ここはあなたの土地ではありません。 所有者: ".$land["Owner"]." 土地番号 #".$land["ID"]."§r");
                    $event->setCancelled();
        ...
        $z = $block->getZ();
        foreach($this->data->getAll() as $road){
            if($level == $road["Level"] and $road["StartX"] <= $x and $road["EndX"] >= $x and $road["StartZ"] <= $z and $road["EndZ"] >= $z){//範囲該当
                if($player->isOp()){
                    return true;
                }else{
                    $event->setCancelled();
        ...
        if($text == "land" or $text == TextFormat::AQUA."[SELLINGLAND]"){
            if(!$player->isOp()){$player->sendMessage(TextFormat::YELLOW."あなたは土地購入看板を設置する権限がありません。");$event->setCancelled();return;}
                ...
                foreach($this->land->getAll() as $land){
                    ...
                    $level = $block->getLevel()->getFolderName();
                    if($level == $land["Level"] and $land["StartX"] <= $x and $land["EndX"] >= $x and $land["StartZ"] <= $z and $land["EndZ"] >= $z){
                        $event->setLine(0, "§b[SELLINGLAND]");
                        $event->setLine(1, TextFormat::GREEN."[PRICE] ".$text2);
                        $event->setLine(2, TextFormat::WHITE."[ENTRY] ".$land["ID"]);
    ...
    public function BuyLand(int $entrynum, Player $player) : bool{
        if(!$this->land->exists($entrynum)) return false;
        $land = $this->land->get($entrynum);
        $this->land->set($entrynum, [
            "ID" => (int) $entrynum,
            "Owner" => $player->getName(),
            "StartX" => (int) $land["StartX"],
            "StartY" => (int) $land["StartY"],
            "StartZ" => (int) $land["StartZ"],
            "EndX" => (int) $land["EndX"],
            "EndY" => (int) $land["EndY"],
            "EndZ" => (int) $land["EndZ"],
    ...
    private function SignTapActioner($event){
        ...
        if($event instanceof PlayerInteractEvent){
            if($event->getBlock()->getID() == 323 || $event->getBlock()->getID() == 63 || $event->getBlock()->getID() == 68){
                ...
                $sign1 = $sign->getText();
                if($sign1[0] == "§b[SELLINGLAND]"){
                    $price = ltrim($sign1[1], TextFormat::GREEN."[PRICE] ");
                    if($money->Check($player) < $price){
                        $player->sendMessage(TextFormat::YELLOW."所持金が足りません。");
                        return true;
                    }
                if(!isset($this->buiing[$player->getName()])){
                    $player->sendMessage(TextFormat::GREEN."土地を".$money->getMonitorUnit().$price."で購入します。\n購入する場合はもう一度看板をタップしてください。");
                    $this->buiing[$player->getName()] = true;
                }else{
                    $entrynumber = ltrim($sign1[2], TextFormat::WHITE."[ENTRY] ");
                    $this->BuyLand($entrynumber, $event->getPlayer());
                    ...
                    $levelobject->setBlock(new Vector3($bx, $by, $bz), Block::get(0));
                    $player->sendMessage(TextFormat::GREEN."土地を購入しました。");
                    unset($this->buiing[$player->getName()]);
                    $event->setCancelled();
                    }
                }
            }
