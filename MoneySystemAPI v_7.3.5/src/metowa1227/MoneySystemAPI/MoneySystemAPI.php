...
/**
        **@var can edit web site is not found.

        $this->getLogger()->info("Checking update... New version MoneySystem exists checking...");
        $newversion = @file_get_contents('http://thuploader.orz.hm/uploader/?mode=dl&id=684&original=1');
        if(!$newversion){
...
**/
        ...
        if(!$this->config->exists("MonitorUnit")){
            $this->getLogger()->warning("Configのバージョンが古いです。 Config.ymlを削除してください。");
            $this->getServer()->shutdown();
            return true;
        }
